...
# Dataset
echinoderma <- data_frame(id = 1:345, e_mathaei = c(rep("black", times = 236), rep("purple" , times = 40), rep("brown", times = 35), rep("green", times = 34)))
...
La visualisation graphique est le point de base dans l'analyse de données. 

### Dénombrement d'oursins perforant dans la baie de Tuléar (Madagascar)

La jeu de données nommé echinoderma est un recensement de différentes variétés d' *Echinoderma mathaei*. 

```{r echinoderma_data, echo = TRUE}
echinoderma <- data_frame(id = 1:345, e_mathaei = c(rep("black", times = 236), 
                                                    rep("purple" , times = 40), 
                                                    rep("brown", times = 35), 
                                                    rep("green", times = 34)))
# Visualisation partielle du tableau de données
head(echinoderma[c(200, 250, 300, 340), ])
```

Les scientifiques produisent le graphique suivant :

```{r}
chart(echinoderma, formula = ~ e_mathaei) +
  geom_bar() +
  coord_flip()
```
...
```{r}
variable <- c("echinoderma", "e_mathaei")
sample(variable)
```
...
```{r}
chart(data = echinoderma, ~ factor(0) %fill=% e_mathaei) +
  geom_bar(width = 1) + 
  coord_polar("y", start = 0) +
  theme_void() +
  scale_fill_viridis_d() +
  labs( fill = expression(italic("Echinoderma mathaei")))
```
...
Les chercheurs souhaitent en plus générer un graphique entierement en français. 

```{r}
echinoderma$e_mathaei <- factor(echinoderma$e_mathaei, levels = c("black", "brown", "green", "purple"), labels = c("Noir", "Brun", "Vert", "Mauve"))

chart(echinoderma, formula = ~ e_mathaei) +
  geom_bar() +
  labs(x = expression(italic("Echinoderma mathaei")), y  = "Dénombrement")
```
...
Les chercheurs proposent ce second graphique pour décire les données
...
Les chercheurs ne sont cependant pas satisfait de ce graphique et réalisent la version suivante :
...
Les chercheurs réalisent un transect de la ville de Nice à la ville de Calvi. Ils vont donc réaliser des prélèvements en 68 stations entre ces deux villes.
...
Ils decident pour finir de représenter les deux graphiques ensembles.
