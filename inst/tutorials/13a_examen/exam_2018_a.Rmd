...
quiz(
  question("Définissez le mode de l'histogramme",
         ...
         answer("multimodal"),
         random_answer_order = TRUE),
  question("Définissez la symétrie de l'histogramme",
...
Les snippets suivants sont mis à votre disposition :
...
quiz(question("Sélectionnez les tests statistiques qui permettent de vérifier l'homogénéité des variances",
         ...
         random_answer_order = TRUE),
     question("Y a t'il homoscédasticité avec une p.value de 0.0003 au seuil $\\alpha$ de 0.05",
              answer("Oui"),
...
# valeur d'intérêt 3
y1 <- y - sample(x = 80:100, size = 1)
```

Dans un champs de maïs (Considérez le nombre de plant de mais comme très grand), dont la taille moyenne est de `r m`  cm et d'écart type de `r s` cm. Quelle est la probabilité d'obtenir une plante de plus de `r q` cm ? Utilisez la fonction adéquate.
...
```{r}
#pnorm(107, mean = 149, sd = 23, lower.tail = FALSE)
# Q1
q1 <- pnorm(q, mean = m, sd = s, lower.tail = FALSE)
q2 <- pnorm(q, mean = m, sd = m-s, lower.tail = TRUE)
...
quiz(question(
  ...
  answer(sprintf("$%s$", round(q1, 2)),correct = TRUE),
  answer(sprintf("$%s$", round(q2, 2))),
  answer(sprintf("$%s$", round(q3, 2))),
  answer(sprintf("$%s$", round(q4, 2))),
  answer("Aucune des réponses proposées"),
  random_answer_order = TRUE
  ),
question(
  sprintf("quelle est la probabilité d'obtenir une plante dont la taille est comprise entre $%s$ et $%s$ cm ?", y, y1), 
  answer(sprintf("$%s$", round(p1, 2)),correct = TRUE),
  answer(sprintf("$%s$", round(p2, 2))),
  answer(sprintf("$%s$", round(p3, 2))),
  answer(sprintf("$%s$", round(p4, 2))),
  answer("Aucune des réponses proposées"),
  random_answer_order = TRUE
  )
)
...
Vous avez à votre disposition les variantes du test de Student présent dans les snippets.
...
```{r qeq_t}
question("Quels sont les paramètres de la distribution associé au test de Student",
         answer("$\\mu$", correct = TRUE),
         answer("$\\sigma / \\sqrt{n}$", correct = TRUE),
...
Suite à l'analyse des résultats ci-dessus, réalisez un test de comparaison multiple de Tukey (si cela est nécéssaire)
...
```{r qeq_tukey}
quiz(
  question("est-il nécessaire de réaliser une test de comparaison multiple de Tukey ? ",
...
## 13. Réalisation d'une boite de dispersion
...
Vous devez retirer les zone (`area`) **B** et **E** et ne garder que les densité strictement supérier à 12.5. 
...
```{r qeq_f}
question("A quels tests statistiques pouvez associer la distribution de Fischer",
