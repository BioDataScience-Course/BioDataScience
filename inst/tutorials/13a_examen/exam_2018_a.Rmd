...
Définissez les conditions d'application d'une distribution binomiale ? Quelles sont les hypothèses de ce test ? 
...
Réalisez l'analyse des résidus suite à cette anova à 1 facteur 
```{r resi_prepa}
is <- data.io::read("iris", package = "datasets", lang = "fr")
anova(anova. <- lm(data = is, petal_length ~ species))
```
...
## 18. Test de comparaison multiple
