...
```{r}
bio <- read(file = "biometrie", package = "BioDataScience")
bio%>.%
  select(., -c(3, 7, 8)) -> bio

kable(bio[c(2:5, 6:8), ], col.names = c("Genre", "Date de naissance", "Masse", "Taille", "Tour de poignet"))
```

On peut filtrer simplement les lignes d'un tableau de données.

```{r}
bio%>.%
  filter(., genre == "F") %>.%
  kable(head(.), col.names = c("Genre", "Date de naissance", "Masse", "Taille", "Tour de poignet"))
```
...
bio%>.%
  ...
  kable(head(.), col.names = c("Genre", "Masse", "Taille"))
```
...
tg <- data::read(file = "ToothGrowth", package = "datasets", lang = "fr")
...
Il devient évident qu'il est plus simple que nos observations de terrains soient rassemblées en un jeu de données structurées pour cela vous allez employer votre première fonction `data_frame()`. Cette dernière va permettre de combiner vos différents vecteurs en un jeu de données.
...
Vous pouvez observez que vous obtenez une nouvelle colonne au sein de votre tableau de données. Réalisez les mêmes opérations sur le jeu de données `man1`
...
Vous pouvez observez que tout comme le tableau de données portant sur les femme, vous obtenez une nouvelle colonne au sein de votre tableau de données portant le nom de BMI soit l'IMC en français.

## 13% de la population adulte mondiale est obèse

Le monde titre que 13% de la populaton mondiale est obèse. Vérifiez cette affirmation avec le jeu de données `bio` qui regroupe  les masse et le taille de 395 personnes adulte vivant sur le territoire belge.
...
Vous pouvez observer que la taille est exprimé en centimètre, il faut en tenir compte lors du cacul de l'IMC.

Un jeu de données réduit est employé pour expliciter les suites d'instructions `bio_ex` qui comprends uniquement 100 observations.
...
## Vérification de l'affirmation de journal le monde
