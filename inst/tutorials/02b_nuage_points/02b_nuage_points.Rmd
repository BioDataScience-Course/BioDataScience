...
library(tidyverse)
# Dataset
urchin <- data::read(file = "urchin_bio", package = "data", lang = "eng")
```
...
La fonction `chart()` requiert comme argument le jeu de données (dataframe, DF), ainsi que la formule à employer YNUM ~ XNUM. Pour réaliser une nuage de point vous devez ajouter la seconde fonction `geom_point()` à l'aide du `+`. 
